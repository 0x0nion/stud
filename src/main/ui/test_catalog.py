...
def test_add_and_remove_cart(page):
    catalog = CatalogSteps(page)
    catalog.login("standard_user", "secret_sauce")
...
def test_remove_product_from_details(page):
    catalog = CatalogSteps(page)
    catalog.login("standard_user", "secret_sauce")

    remove_btn = catalog.remove_from_cart("Test.allTheThings() T-Shirt (Red)")

    expect(remove_btn).to_have_text("Add to cart")
