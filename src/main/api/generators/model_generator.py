...
from src.main.api.models.create_user_request import UserRole
...
class RandomModelGenerator:
            ...
            if get_origin(annotated_type) is Annotated:
                ...
                for ann in annotations:
                    if isinstance(ann, CreationRule):
                        rule = ann
            if rule:
                value = RandomModelGenerator._generate_from_rexeg(rule.regex, actual_type)
            else:
    ...
    @staticmethod
    def _generate_from_rexeg(regex: str, field_type: type) -> Any:
